...
import (
	...
	"github.com/quantosnetwork/karod/p2p/grpc"
	"github.com/quantosnetwork/karod/p2p/proto"
...
func (d *discovery) setup(bootnodes []*peer.AddrInfo) error {
	...
	keyID := kb.ConvertPeerID(d.srv.h.ID())
	...
	d.routingTable.PeerAdded = func(p peer.ID) {
		info := d.srv.h.Peerstore().PeerInfo(p)
...
func (d *discovery) addToTable(node *peer.AddrInfo) error {
	...
	d.srv.h.Peerstore().AddAddr(node.ID, node.Addrs[0], peerstore.AddressTTL)
...
func (d *discovery) FindPeers(
	...
	for _, id := range closer {
		...
		if id != from {
			if info := d.srv.h.Peerstore().PeerInfo(id); len(info.Addrs) > 0 {
